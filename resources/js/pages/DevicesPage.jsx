...
export function DevicesPage() {
    ...
    const devices = useDevices({
        ...
        status:
            ...
            undefined,
        laboratory_id:
            list.values.laboratory ||
            undefined,
    });

    const laboratories =
        useLaboratories(scope.params);

    const all =
        devices.data?.data ?? [];

    // The API has no unattributed filter,
    // so this narrows the current page only.
    const rows =
        list.values.attribution ===
        'required'
            ? all.filter(
                  (device) =>
                      (device
    ...
    /*
     * A school page links here with ?device=<id>.
     * Open the device once it is available,
     * then clear the parameter so refreshes are not sticky.
     */
                            ...
                            >
                                ...
                                <Cell className="tabular-nums">
                                    {device.assignment_capacity
                                        ? `${device.assignment_capacity.used}/${device.assignment_capacity.maximum}`
                                        : '—'}
                                </Cell>
    ...
    async function decommission() {
        try {
            ...
            showToast(
                ...
                {
                    description: `${device.asset_tag} was removed from the register.`,
                },
            );
            ...
            toastError(
                error,
                'The device could not be decommissioned',
            );
        }
    }
    ...
    return (
        <>
            <Modal
                ...
                subtitle={
                    isEdit
                        ? 'Update the enrolment record for this computer.'
                        : 'Enrol a computer before a learner uses it.'
                }
                size="md"
                footer={
                    <>
            ...
            >
                ...
                >
                    <Field
                        label="Asset tag"
                        required
                        hint="The label physically fixed to the computer."
                        error={firstError(
                            errors,
                            'asset_tag',
                        )}
                    ...
                    <div className="grid gap-4 sm:grid-cols-3">
                        ...
                        <Field
                            label="Use"
                            hint="Learner-use devices require attribution."
                            error={firstError(
                                errors,
                                'usage_type',
                            )}
...
function DeviceModal({
    ...
    const atCapacity =
        assigned.length >=
        MAX_LEARNERS_PER_DEVICE;

    const canAssign =
        capabilities?.assignLearners;

    const usageLabel = device
        ? (USAGE_TYPES.find(
              ([value]) =>
                  value ===
                  device.usage_type,
    ...
    async function submitAssignment(
        ...
        try {
            ...
            showToast(
                'Learner attributed to device',
                {
                    description: `${device.asset_tag} now carries an accountable learner.`,
                },
            );
            ...
            toastError(
                error,
                'The learner could not be assigned',
            );
        }
    }
    ...
    async function confirmRemoval() {
        try {
            ...
            showToast(
                ...
                {
                    description:
                        'The change was written to the county audit log.',
                },
            );
        } catch (error) {
            toastError(
                error,
                'The attribution could not be removed',
            );
        } finally {
            setPendingRemoval(
                null,
    ...
    async function submitSession(
        ...
        try {
            ...
            showToast(
                `${sessionFor.name} signed in`,
                {
                    description: `Active learner session opened on ${device.asset_tag}.`,
                },
            );
    ...
    async function closeSession() {
        try {
            await endSession.mutateAsync(
                device.active_session.id,
            );

            showToast(
                'Session ended',
                {
                    description: `${device.active_session.learner} was signed out of ${device.asset_tag}.`,
                },
            );
        } catch (error) {
            toastError(
                    ...
                    <div className="max-h-[72vh] overflow-y-auto pr-1">
                        ...
                        <div className="flex min-h-9 items-center justify-between gap-3">
                            <div className="flex min-w-0 flex-wrap items-center gap-2">
                                ...
                                {device.active_session && (
                                    <span className="inline-flex items-center gap-1.5 rounded-full bg-success-soft px-2 py-1 text-[10px] font-semibold text-success">
                                        <span
                                            aria-hidden
                                            className="h-1.5 w-1.5 rounded-full bg-success"
                                        />
                        ...
                        <div className="mt-3 grid grid-cols-2 overflow-hidden rounded-lg border border-border bg-surface-muted/30 sm:grid-cols-4">
                            ...
                            <DeviceCompactStat
                                label="Attribution"
                                value={`${assigned.length}/${MAX_LEARNERS_PER_DEVICE}`}
                                tone={
                                    assigned.length ===
                                        0 &&
                                    device.usage_type ===
                        ...
                        {/* Main content */}
                        <div className="mt-4 grid min-w-0 gap-4 md:grid-cols-[250px_minmax(0,1fr)]">
                            {/* Operational rail */}
                            <aside className="min-w-0 space-y-4">
                                {/* Device details */}
                                <section>
                                    <DeviceSectionHeading title="Device details" />
                                ...
                                {/* Active session */}
                                <section>
                                    <DeviceSectionHeading title="Active session" />

                                    {device.active_session ? (
                                        <div className="mt-2 rounded-lg border border-success/30 bg-success-soft/50 p-3">
                                            <div className="flex items-start justify-between gap-2">
                                                <div className="min-w-0">
                                                    ...
                                                    <p className="mt-0.5 truncate text-[10px] text-text-secondary">
                                                        {device.active_session.identity_source.replaceAll(
                                                            '_',
                                                            ' ',
                                                        )}
                                                    </p>
                                ...
                                {/* Attribution */}
                                <section>
                                    <DeviceSectionHeading
                                        title="Assigned learners"
                                        count={`${assigned.length}/${MAX_LEARNERS_PER_DEVICE}`}
                                    />
                                                    ...
                                                    >
                                                        <div className="flex items-start justify-between gap-2">
                                                            ...
                                                            {canAssign && (
                                                                <button
                                                                    ...
                                                                    className="shrink-0 rounded-md p-1 text-text-muted transition-colors hover:bg-danger-soft hover:text-danger"
                                                                >
                                                                    <MinusIcon
                                                                        size={
                                                                            13
                                                        ...
                                                        <div className="mt-2 flex items-center justify-between gap-2">
                                                            <Link
                                                                ...
                                                                className="text-[9px] font-semibold text-brand hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand"
                                                            >
                                                                View
                                                                learner
                                                            </Link>
                                                            ...
                                                            {capabilities?.manageSessions &&
                                                                !device.active_session && (
                                                                    <button
                                                                        ...
                                                                        className="flex items-center gap-1 rounded-md px-1.5 py-1 text-[9px] font-semibold text-brand transition-colors hover:bg-brand-soft"
                                                                    >
                                                                        <PlayIcon
                                                                            size={
                                                                                10
                                    ...
                                    {assigning && (
                                        <form
                                            ...
                                            className="mt-2 space-y-2.5 rounded-lg border border-brand/25 bg-brand-soft/20 p-2.5"
                                        >
                                            <Field
                                                label="Learner"
                                                required
                                    ...
                                    {sessionFor && (
                                        <form
                                            ...
                                            className="mt-2 space-y-2.5 rounded-lg border border-brand/30 bg-brand-soft/30 p-2.5"
                                        >
                                            <div>
                                                <p className="text-[9px] font-semibold uppercase tracking-[0.07em] text-text-muted">
                                                    Start
                                                    session
                                                </p>
                                            ...
                                            >
                                                ...
                                                >
                                                    <option value="school_pin">
                                                        School
                                                        PIN
                                                    </option>

                                                    <option value="google_workspace">
                                                        Google
                                                        Workspace
                                                    </option>
...
function DeviceMetaRow({
    ...
    return (
        <div className="flex items-center justify-between gap-3 border-b border-border px-3 py-2 last:border-b-0">
            <span className="shrink-0 text-[9px] font-medium uppercase tracking-[0.05em] text-text-muted">
                {label}
            </span>

            <span
                className={`min-w-0 truncate text-right text-[10px] font-medium text-text ${
                    mono
                        ? 'font-mono'
                        : ''
                }`}
